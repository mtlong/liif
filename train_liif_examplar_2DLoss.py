...
def train(train_loader, model, optimizer):
    ...
    sample_w = int(math.sqrt(config['train_dataset']['wrapper']['args']['sample_q']))
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
        ...
        loss = loss_fn(pred.view(-1, 3, sample_w, sample_w), gt.view(-1, 3, sample_w, sample_w))        
        if use_lpips:
...
if __name__ == '__main__':
    ...
    train_temp_folder = config.get('train_dataset')['dataset']['args']['root_path']
    val_temp_folder = config.get('val_dataset')['dataset']['args']['root_path']
    print("Temp training folder: " + train_temp_folder)
    train_data_root = config.get('train_data_root')
    gt_data_root = config.get('gt_data_root')
    train_data_folder = os.path.join(train_data_root, args.tag) 
    os.system("rm -f " + train_temp_folder + "/*.*")
    os.system("cp " + train_data_folder + "/" + "*.* " + train_temp_folder)
    os.system("rm -f " + val_temp_folder + "/*.*")
    os.system("cp " + gt_data_root + "/" + args.tag + " " + val_temp_folder)
